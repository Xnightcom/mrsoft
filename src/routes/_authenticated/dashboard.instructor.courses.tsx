      ...
      >
        <Tabs defaultValue="students" className="mt-2">
          <TabsList className="bg-white/5 border border-white/10 grid grid-cols-4 w-full h-10">
            <TabsTrigger value="students" className="data-[state=active]:bg-[#CC0000] text-xs">
              Students
            </TabsTrigger>
            <TabsTrigger value="books" className="data-[state=active]:bg-[#CC0000] text-xs">
              Books
            </TabsTrigger>
            <TabsTrigger value="attendance" className="data-[state=active]:bg-[#CC0000] text-xs">
              Attendance
            </TabsTrigger>
            <TabsTrigger value="assignments" className="data-[state=active]:bg-[#CC0000] text-xs">
              Assignments
            </TabsTrigger>
          </TabsList>
            ...
            <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-3">
              {enrollments.length === 0 ? (
                ...
                enrollments.map((enr: any) => (
                  <div
                    key={enr.id}
                    className="bg-white/5 border border-white/10 p-4 rounded-lg flex items-center justify-between gap-4"
                  >
          ...
          <TabsContent value="attendance" className="mt-4 space-y-4">
            <div className="border border-white/10 rounded-lg p-4 bg-[#0A0A0A]">
              ...
              <div className="space-y-3 mb-6 max-h-[40vh] overflow-y-auto pr-2">
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() =>
                          setAttendanceState((p) => ({ ...p, [enr.student_id]: true }))
                        }
                      >
                        ✓ Present
                      </Button>
                      <Button
                        ...
                        onClick={() =>
                          setAttendanceState((p) => ({ ...p, [enr.student_id]: false }))
                        }
                      >
                        ✗ Absent
                      </Button>
                ...
                {enrollments.length === 0 && (
                  <p className="text-center text-white/50 text-sm">No students enrolled.</p>
                )}
              </div>
              ...
              <div className="flex justify-between items-center border-t border-white/10 pt-4">
                ...
                <Button
                  ...
                  className="bg-[#CC0000] hover:bg-[#CC0000]/80 text-white h-8 text-xs px-6"
                >
              ...
              <div className="mt-4 p-3 bg-white/5 rounded flex justify-between text-xs text-white/70 border border-white/10">
                <span>
                  Total Students: <strong className="text-white">{enrollments.length}</strong>
                </span>
                <span>
                  Present Today:{" "}
                  <strong className="text-green-500">
                    {Object.values(attendanceState).filter((v) => v === true).length}
                  </strong>
                </span>
                <span>
                  Absent Today:{" "}
                  <strong className="text-red-500">
                    {Object.values(attendanceState).filter((v) => v === false).length}
                  </strong>
                </span>
                <span>
                  Unmarked:{" "}
                  <strong className="text-white">
                    {enrollments.length - Object.values(attendanceState).length}
                  </strong>
                </span>
              </div>
            </div>
          ...
          <TabsContent value="assignments" className="mt-4 space-y-4">
            <div className="grid grid-cols-5 gap-4 h-[60vh]">
              <div className="col-span-2 border border-white/10 rounded-lg p-4 bg-[#0A0A0A] overflow-y-auto">
                <h3 className="font-bold text-white mb-4 text-sm border-b border-white/10 pb-2">
                  Create Assignment
                </h3>
                <form
                  onSubmit={(e) => {
                ...
                >
                  <div>
                    ...
                    <Input
                      ...
                      className="h-8 text-xs bg-[#060606] border-white/20"
                    />
                  </div>
                  <div>
                    ...
                    <Textarea
                      ...
                      className="min-h-[80px] text-xs bg-[#060606] border-white/20"
                    />
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setAssignmentForm({ ...assignmentForm, due_date: e.target.value })
                        }
                        className="h-8 text-xs bg-[#060606] border-white/20"
                      />
                    </div>
                  ...
                  <Button
                    ...
                    className="w-full h-8 text-xs bg-[#1A6B1A] hover:bg-[#1A6B1A]/80 text-white"
                  >
                    Create
                  </Button>
                </form>
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-[10px] text-white/50">
                            Student: {assn.student?.full_name}
                          </p>
                        </div>
                      ...
                      {assn.submission_text && !assn.grade && (
                        <div className="flex gap-2 items-center">
                          <Input
                            ...
                            type="number"
                            placeholder="Grade"
                            className="w-20 h-7 text-xs bg-[#060606]"
                          />
                          <Input
                            id={`fb-${assn.id}`}
                            placeholder="Feedback..."
                            className="flex-1 h-7 text-xs bg-[#060606]"
                          />
                          <Button
                            size="sm"
                            ...
                            onClick={() => {
                              const grade = parseInt(
                                (document.getElementById(`grade-${assn.id}`) as HTMLInputElement)
                                  .value,
                              );
                              const fb = (
                                document.getElementById(`fb-${assn.id}`) as HTMLInputElement
                              ).value;
                              if (!isNaN(grade)) {
