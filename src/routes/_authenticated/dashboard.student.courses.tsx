          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[rgba(26,107,26,0.2)] pb-4">
              <div>
                ...
                {activeEnrollment?.instructor_notes && (
                  <div className="mt-2 text-xs bg-[#1A6B1A]/10 border border-[#1A6B1A]/20 p-2 rounded text-green-400">
                    <strong className="text-white">Instructor Notes:</strong>{" "}
                    {activeEnrollment.instructor_notes}
                  </div>
                )}
              </div>
              <div className="w-full md:w-64 bg-[#0F0F0F] border border-[rgba(26,107,26,0.3)] rounded-xl p-4">
                ...
                <p className="text-[10px] text-white/40 mt-1 text-center italic">
                  Progress updated by instructor
                </p>
              </div>
            </div>
            ...
            <Tabs defaultValue="syllabus" className="mt-4">
              <TabsList className="bg-white/5 border border-white/10 grid grid-cols-4 w-full h-10">
                <TabsTrigger value="syllabus" className="data-[state=active]:bg-[#CC0000] text-xs">
                  Syllabus
                </TabsTrigger>
                ...
                <TabsTrigger
                  value="attendance"
                  className="data-[state=active]:bg-[#CC0000] text-xs"
                >
                  Attendance
                </TabsTrigger>
                <TabsTrigger
                ...
                {lessonsLoading ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-14 bg-white/5 border border-[rgba(26,107,26,0.2)] rounded-lg animate-pulse"
                      />
                    ))}
                  </div>
                ) : totalLessons === 0 ? (
                  ...
                  <div className="space-y-2 max-h-[50vh] overflow-y-auto">
                    {attendance.length === 0 ? (
                      ...
                      attendance.map((rec: any) => (
                        <div
                          key={rec.id}
                          className="flex justify-between items-center text-xs p-3 bg-white/5 rounded border border-white/5"
                        >
                          <span className="text-white/70 font-medium">
                            {new Date(rec.date).toLocaleDateString()}
                          </span>
                          <span
                            className={`capitalize font-bold px-2 py-0.5 rounded ${rec.status === "present" ? "bg-green-500/10 text-green-500" : rec.status === "late" ? "bg-yellow-500/10 text-yellow-500" : "bg-red-500/10 text-red-500"}`}
                          >
                            {rec.status}
                          </span>
                        </div>
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {assignments.length === 0 ? (
                    ...
                    assignments.map((assn: any) => (
                      <div
                        key={assn.id}
                        className="p-4 bg-[#0A0A0A] rounded-lg border border-white/10"
                      >
                          ...
                          <div>
                            ...
                            {assn.due_date && (
                              <p className="text-[10px] text-red-400 mt-1">
                                Due: {new Date(assn.due_date).toLocaleDateString()}
                              </p>
                            )}
                          </div>
                          <span className="text-[10px] bg-white/10 px-2 py-0.5 rounded text-white/70 whitespace-nowrap">
                            Max: {assn.max_grade}
                          </span>
                        </div>
                        ...
                        {assn.grade !== null ? (
                          ...
                          <div className="mt-4 space-y-2">
                            ...
                            <Button
                              ...
                              disabled={
                                submitAssignment.isPending || !submissionTexts[assn.id]?.trim()
                              }
            ...
            {coursesLoading ? (
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-60 bg-white/5 border border-[rgba(26,107,26,0.2)] rounded-xl animate-pulse"
                  />
                ))}
              </div>
            ) : enrolls.length === 0 ? (
