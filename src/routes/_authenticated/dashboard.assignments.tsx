...
function AssignmentsPage() {
  ...
  const { data } = useQuery({
    ...
    enabled: !!user.userId,
    queryFn: async () => {
      const { data, error } = await supabase.from("assignments").select("*, course:courses(title)").order("deadline", { ascending: true });
          ...
          {!data?.length ? (
            ...
            <Card key={a.id} className="hover:shadow-elegant transition-smooth">
              <CardContent className="p-6 flex items-center justify-between gap-4">
                <div>
                  <div className="text-xs text-muted-foreground">{(a.course as { title: string } | null)?.title}</div>
                ...
                <div className="text-right text-sm">
                  {a.deadline && <Badge variant="outline">Due {new Date(a.deadline).toLocaleDateString()}</Badge>}
                </div>
              </CardContent>
            </Card>
          ))}
