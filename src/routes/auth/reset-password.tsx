          ...
          <CardContent className="p-0">
            {done ? (
              <div className="text-center py-6 animate-in fade-in zoom-in duration-500">
                ...
                <p className="text-white/80 mb-6 text-sm">Your new password has been set. Redirecting to sign in...</p>
                <Button onClick={() => navigate({ to: '/auth' })} className="w-full bg-[#1A6B1A] hover:bg-[#1A6B1A]/80 text-white">Go to Sign in</Button>
              </div>
            ) : (
              <>
                <div className="flex flex-col items-center mb-8">
                  <h1 className="text-2xl font-bold text-white">Set new password</h1>
                  <p className="text-sm text-white/60 mt-2 text-center">Please enter your new password below.</p>
                </div>
                ...
                <form onSubmit={handleReset} className="space-y-4">
                  ...
                  <Button type="submit" className="w-full bg-[#CC0000] hover:bg-[#CC0000]/80 text-white mt-6" disabled={loading}>
                    {loading ? "Updating..." : "Update Password"}
                  </Button>
                </form>
                
                <p className="mt-8 text-center text-xs text-white/40">
                  <Link to="/auth" className="hover:underline hover:text-white/70 transition-smooth">← Back to sign in</Link>
                </p>
              </>
            )}
          </CardContent>
