...
export function DashboardLayout({ profile: propProfile, children }: Props) {
  ...
  const currentLink = links.find(
    (l) =>
      ...
      (l.to !== `/dashboard/${profile?.role || "client"}` && location.pathname.startsWith(l.to)),
  );
          ...
          <img
            ...
            onError={(e) => {
              ...
              (e.target as HTMLImageElement).parentElement!.innerHTML =
                '<span class="font-bold text-xl tracking-wider">MRsoft</span>';
            }}
          />
          <button
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 flex flex-col gap-1">
          {links.map((link) => {
            const isActive =
              ...
              (link.to !== `/dashboard/${profile?.role || "client"}` &&
